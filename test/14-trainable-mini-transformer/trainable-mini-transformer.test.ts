import { join } from 'node:path'

import { afterEach, describe, expect, it } from 'vitest'

import { createCharacterTokenizer } from '../../src/modules/01-tokenizer-simple/index.js'
import { createTokenDataset, loadTextFile } from '../../src/modules/02-dataset-loader/index.js'
import { createNextTokenExamples } from '../../src/modules/08-training-loop-cpu/index.js'
import {
    computeMiniTransformerAverageLoss,
    createTrainableMiniTransformer,
    disposeTrainableMiniTransformer,
    generateMiniTransformerText,
    generateMiniTransformerTokenIds,
    predictMiniTransformerLogits,
    predictMiniTransformerNextToken,
    predictMiniTransformerProbabilities,
    trainMiniTransformer,
    type TrainableMiniTransformer,
} from '../../src/modules/14-trainable-mini-transformer/index.js'

const modelsToDispose: TrainableMiniTransformer[] = []

afterEach(() => {
    for (const model of modelsToDispose.splice(0)) {
        disposeTrainableMiniTransformer(model)
    }
})

describe('createTrainableMiniTransformer', () => {
    it('crée les variables aux bonnes shapes', () => {
        const model = trackModel(
            createTrainableMiniTransformer({
                contextLength: 4,
                embeddingDimension: 8,
                feedForwardDimension: 16,
                vocabularySize: 10,
            }),
        )

        expect(model.tokenEmbeddings.shape).toEqual([10, 8])
        expect(model.positionEmbeddings.shape).toEqual([4, 8])
        expect(model.queryWeights.shape).toEqual([8, 8])
        expect(model.keyWeights.shape).toEqual([8, 8])
        expect(model.valueWeights.shape).toEqual([8, 8])
        expect(model.attentionOutputWeights.shape).toEqual([8, 8])
        expect(model.feedForwardInputWeights.shape).toEqual([8, 16])
        expect(model.feedForwardInputBias.shape).toEqual([16])
        expect(model.feedForwardOutputWeights.shape).toEqual([16, 8])
        expect(model.feedForwardOutputBias.shape).toEqual([8])
        expect(model.outputWeights.shape).toEqual([8, 10])
        expect(model.outputBias.shape).toEqual([10])
    })

    it('rejette des dimensions invalides', () => {
        expect(() =>
            createTrainableMiniTransformer({
                contextLength: 4,
                embeddingDimension: 8,
                feedForwardDimension: 16,
                vocabularySize: 0,
            }),
        ).toThrow('vocabularySize doit être un entier strictement positif.')
        expect(() =>
            createTrainableMiniTransformer({
                contextLength: 0,
                embeddingDimension: 8,
                feedForwardDimension: 16,
                vocabularySize: 10,
            }),
        ).toThrow('contextLength doit être un entier strictement positif.')
        expect(() =>
            createTrainableMiniTransformer({
                contextLength: 4,
                embeddingDimension: 0,
                feedForwardDimension: 16,
                vocabularySize: 10,
            }),
        ).toThrow('embeddingDimension doit être un entier strictement positif.')
        expect(() =>
            createTrainableMiniTransformer({
                contextLength: 4,
                embeddingDimension: 8,
                feedForwardDimension: 0,
                vocabularySize: 10,
            }),
        ).toThrow('feedForwardDimension doit être un entier strictement positif.')
    })
})

describe('predictMiniTransformerLogits', () => {
    it('retourne un logit par token', () => {
        const model = trackModel(createSmallModel())
        const logits = predictMiniTransformerLogits(model, [0, 1])

        expect(logits.shape).toEqual([5])

        logits.dispose()
    })

    it("rejette un contexte qui n'a pas contextLength tokens", () => {
        const model = trackModel(createSmallModel())

        expect(() => predictMiniTransformerLogits(model, [0])).toThrow(
            'Le contexte doit contenir 2 tokens.',
        )
    })

    it('rejette un token de contexte hors vocabulaire', () => {
        const model = trackModel(createSmallModel())

        expect(() => predictMiniTransformerLogits(model, [0, 5])).toThrow(
            'inputTokenIds[1] doit être un entier entre 0 et 4.',
        )
    })
})

describe('predictMiniTransformerProbabilities', () => {
    it('retourne une distribution normalisée', () => {
        const model = trackModel(createSmallModel())
        const probabilities = predictMiniTransformerProbabilities(model, [0, 1])
        const sum = probabilities.reduce((total, probability) => total + probability, 0)

        expect(probabilities).toHaveLength(5)
        expect(sum).toBeCloseTo(1)
    })
})

describe('predictMiniTransformerNextToken', () => {
    it('retourne un id valide', () => {
        const model = trackModel(createSmallModel())
        const tokenId = predictMiniTransformerNextToken(model, [0, 1])

        expect(tokenId).toBeGreaterThanOrEqual(0)
        expect(tokenId).toBeLessThan(5)
    })
})

describe('generation', () => {
    it('generateMiniTransformerTokenIds ajoute exactement maxNewTokens tokens', () => {
        const model = trackModel(createSmallModel())
        const result = generateMiniTransformerTokenIds(model, [0, 1], { maxNewTokens: 3 })

        expect(result.generatedTokenIds).toHaveLength(3)
        expect(result.steps).toHaveLength(3)
        expect(result.tokenIds).toHaveLength(5)
    })

    it('generateMiniTransformerText retourne un texte commençant par le prompt', () => {
        const tokenizer = createCharacterTokenizer('abc')
        const model = trackModel(
            createTrainableMiniTransformer({
                contextLength: 2,
                embeddingDimension: 4,
                feedForwardDimension: 8,
                vocabularySize: tokenizer.vocabularySize,
            }),
        )
        const result = generateMiniTransformerText(model, tokenizer, 'ab', { maxNewTokens: 2 })

        expect(result.text.startsWith('ab')).toBe(true)
        expect(result.generatedTokenIds).toHaveLength(2)
    })
})

describe('computeMiniTransformerAverageLoss', () => {
    it('retourne une loss positive', () => {
        const model = trackModel(
            createTrainableMiniTransformer({
                contextLength: 2,
                embeddingDimension: 4,
                feedForwardDimension: 8,
                vocabularySize: 4,
            }),
        )
        const loss = computeMiniTransformerAverageLoss(model, [
            { inputTokenIds: [0, 1], targetTokenId: 2 },
            { inputTokenIds: [1, 2], targetTokenId: 3 },
        ])

        expect(loss).toBeGreaterThan(0)
    })
})

describe('trainMiniTransformer', () => {
    it('réduit la loss sur un dataset répétitif simple', () => {
        const model = trackModel(
            createTrainableMiniTransformer({
                contextLength: 2,
                embeddingDimension: 4,
                feedForwardDimension: 8,
                seed: 42,
                vocabularySize: 3,
            }),
        )
        const examples = [
            { inputTokenIds: [0, 1], targetTokenId: 2 },
            { inputTokenIds: [0, 1], targetTokenId: 2 },
            { inputTokenIds: [0, 1], targetTokenId: 2 },
            { inputTokenIds: [1, 0], targetTokenId: 1 },
        ]

        const history = trainMiniTransformer(model, examples, {
            epochs: 20,
            learningRate: 0.05,
        })

        expect(history.finalLoss).toBeLessThan(history.initialLoss)
    })

    it('fonctionne avec tokenizer, mini corpus, dataset loader et exemples du module 8', async () => {
        const rawText = await loadTextFile(join(process.cwd(), 'data', 'tiny-corpus.txt'))
        const tokenizer = createCharacterTokenizer(rawText)
        const dataset = createTokenDataset(rawText, tokenizer)
        const examples = createNextTokenExamples(dataset.trainTokenIds, { contextLength: 4 })
        const model = trackModel(
            createTrainableMiniTransformer({
                contextLength: 4,
                embeddingDimension: 8,
                feedForwardDimension: 16,
                seed: 14,
                vocabularySize: tokenizer.vocabularySize,
            }),
        )

        const history = trainMiniTransformer(model, examples, {
            epochs: 2,
            learningRate: 0.03,
        })

        expect(history.epochs).toHaveLength(2)
        expect(history.finalLoss).toBeGreaterThan(0)
    })
})

describe('disposeTrainableMiniTransformer', () => {
    it('peut être appelé sans erreur', () => {
        const model = createSmallModel()

        expect(() => {
            disposeTrainableMiniTransformer(model)
        }).not.toThrow()
    })
})

function createSmallModel(): TrainableMiniTransformer {
    return createTrainableMiniTransformer({
        contextLength: 2,
        embeddingDimension: 4,
        feedForwardDimension: 8,
        vocabularySize: 5,
    })
}

function trackModel(model: TrainableMiniTransformer): TrainableMiniTransformer {
    modelsToDispose.push(model)

    return model
}
