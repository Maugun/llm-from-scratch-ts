...
} from './checkpoint-versioning.js'
export type {
    CheckpointVersionInfo,
    CheckpointVersionPlan,
    CheckpointVersionPlanOptions,
} from './checkpoint-versioning.js'

export type SmallLanguageModelOptions = {
...
export type SmallLanguageModelLayer = {
...
export type SmallLanguageModel = {
    readonly vocabularySize: number
    readonly contextLength: number
    readonly embeddingDimension: number
    readonly feedForwardDimension: number
    ...
    readonly layers: readonly SmallLanguageModelLayer[]
    readonly outputWeights: tf.Variable
    readonly outputBias: tf.Variable
}

export type SmallLanguageModelTrainingOptions = {
    readonly epochs: number
    readonly learningRate: number
    readonly batchSize?: number
    readonly maxTrainBatchesPerEpoch?: number
    readonly maxValidationBatches?: number
    readonly onProgress?: (progress: SmallLanguageModelTrainingProgress) => void
    readonly batchOrder?: SmallLanguageModelTrainingBatchOrder
    readonly shuffleSeed?: number
}

export type SmallLanguageModelTrainingBatchOrder = 'sequential' | 'shuffled'

export type SmallLanguageModelTrainingProgress = {
...
export type SmallLanguageModelEpochMetrics = {
    readonly epoch: number
...
export type SmallLanguageModelTrainingHistory = {
    ...
    readonly epochs: readonly SmallLanguageModelEpochMetrics[]
}

export type SmallLanguageModelEvaluationOptions = {
    readonly batchSize: number
    readonly maxBatches?: number
}

export type SmallLanguageModelEvaluationMetrics = {
...
export type SmallLanguageModelGenerationOptions = {
...
export type SmallLanguageModelGenerationStep = {
...
export type SmallLanguageModelTextGenerationTokenizer = {
    readonly encode: (text: string) => number[]
    readonly decode: (tokenIds: number[]) => string
}

export type SmallLanguageModelTextGenerationResult = {
    ...
    readonly steps: readonly SmallLanguageModelGenerationStep[]
}

export type SmallLanguageModelCheckpointMetadata = {
    readonly version: 1
    readonly createdAt: string
    readonly modelOptions: SmallLanguageModelOptions
    readonly extra?: Record<string, unknown>
    readonly variables: readonly SmallLanguageModelCheckpointVariable[]
}

export type SmallLanguageModelCheckpointVariable = {
...
export type SaveSmallLanguageModelCheckpointOptions = {
    readonly extra?: Record<string, unknown>
}

const defaultSeed = 18
...
export function createSmallLanguageModel(options: SmallLanguageModelOptions): SmallLanguageModel {
    validateModelOptions(options)

    const seed = options.seed ?? defaultSeed
    const variablePrefix = `smallLm_${String(modelInstanceCounter++)}`
...
export function predictSmallLanguageModelProbabilities(
    model: SmallLanguageModel,
    inputTokenIds: readonly number[],
): readonly number[] {
    const probabilities = tf.tidy(() =>
        tf.softmax(predictSmallLanguageModelLogits(model, inputTokenIds)),
    )
...
export function selectSmallLanguageModelNextToken(
    model: SmallLanguageModel,
    inputTokenIds: readonly number[],
    samplingOptions: SamplingSelectionOptions,
): number {
    const probabilities = predictSmallLanguageModelProbabilities(model, inputTokenIds)

    return selectNextToken(probabilities, samplingOptions)
}

export function generateSmallLanguageModelText(
    model: SmallLanguageModel,
    tokenizer: SmallLanguageModelTextGenerationTokenizer,
    prompt: string,
    options: SmallLanguageModelGenerationOptions,
): SmallLanguageModelTextGenerationResult {
    ...
    const steps: SmallLanguageModelGenerationStep[] = []

    for (let step = 1; step <= options.maxNewTokens; step++) {
        const contextTokenIds = tokenIds.slice(-model.contextLength)
        const probabilities = predictSmallLanguageModelProbabilities(model, contextTokenIds)
...
export function evaluateSmallLanguageModel(
    model: SmallLanguageModel,
    tokenIds: readonly number[],
    options: SmallLanguageModelEvaluationOptions,
): SmallLanguageModelEvaluationMetrics {
    ...
    if (evaluatedExamples === 0) {
        throw new Error('Aucun exemple disponible pour évaluer le modèle.')
    }
...
export function trainSmallLanguageModel(
    model: SmallLanguageModel,
    pipeline: LongCorpusPipeline,
    options: SmallLanguageModelTrainingOptions,
): SmallLanguageModelTrainingHistory {
    ...
    const epochMetrics: SmallLanguageModelEpochMetrics[] = []
    ...
    for (let epoch = 1; epoch <= options.epochs; epoch++) {
        ...
        for (const batch of iterateTrainingBatches(
            ...
            options.batchOrder ?? 'sequential',
            (options.shuffleSeed ?? defaultSeed) + epoch,
        )) {
            if (
                options.maxTrainBatchesPerEpoch !== undefined &&
                trainedBatches >= options.maxTrainBatchesPerEpoch
            ) {
                break
            }
        ...
        const validation = evaluateSmallLanguageModel(model, pipeline.validationTokenIds, {
            batchSize,
            maxBatches: maxValidationBatches,
        })
    ...
    const finalValidation = evaluateSmallLanguageModel(model, pipeline.validationTokenIds, {
        batchSize,
        maxBatches: maxValidationBatches,
    })
...
export async function saveSmallLanguageModelCheckpoint(
    model: SmallLanguageModel,
    directoryPath: string,
    options: SaveSmallLanguageModelCheckpointOptions = {},
): Promise<SmallLanguageModelCheckpointMetadata> {
    await mkdir(directoryPath, { recursive: true })

    const variables = getNamedVariables(model)
    const checkpointVariables: SmallLanguageModelCheckpointVariable[] = []
    ...
    const metadata: SmallLanguageModelCheckpointMetadata = {
...
export async function loadSmallLanguageModelCheckpoint(
    directoryPath: string,
): Promise<SmallLanguageModel> {
    const metadata = await readCheckpointMetadata(directoryPath)
    const model = createSmallLanguageModel(metadata.modelOptions)
...
export function disposeSmallLanguageModel(model: SmallLanguageModel): void {
...
function createLayer(
    options: SmallLanguageModelOptions,
    seed: number,
    layerIndex: number,
    variablePrefix: string,
): SmallLanguageModelLayer {
    return {
...
function predictSmallLanguageModelLogits(
    model: SmallLanguageModel,
    inputTokenIds: readonly number[],
): tf.Tensor1D {
...
function computeBatchLogits(model: SmallLanguageModel, inputTokenIds: tf.Tensor2D): tf.Tensor2D {
    return tf.tidy(() => {
        const contextualVectors = computeContextualVectors(model, inputTokenIds)
        const lastVector = contextualVectors
...
function computeContextualVectors(
    model: SmallLanguageModel,
...
function applyTransformerLayer(
    model: SmallLanguageModel,
    layer: SmallLanguageModelLayer,
    inputVectors: tf.Tensor3D,
): tf.Tensor3D {
    return tf.tidy(() => {
        const queries = projectSequence(inputVectors, layer.queryWeights, model.embeddingDimension)
        const keys = projectSequence(inputVectors, layer.keyWeights, model.embeddingDimension)
        const values = projectSequence(inputVectors, layer.valueWeights, model.embeddingDimension)
        const scores = tf
...
function createBatchOptions(model: SmallLanguageModel, batchSize: number): NextTokenBatchOptions {
    return {
        batchSize,
...
function* iterateTrainingBatches(
    ...
    batchOrder: SmallLanguageModelTrainingBatchOrder,
    seed: number,
    ...
    for (const [shuffledBatchIndex, originalBatchIndex] of batchIndices.entries()) {
        const startExampleIndex = originalBatchIndex * options.batchSize
...
function getTrainableVariables(model: SmallLanguageModel): tf.Variable[] {
...
function getModelOptions(model: SmallLanguageModel): SmallLanguageModelOptions {
    return {
        contextLength: model.contextLength,
...
function validateCheckpointMetadata(value: unknown): SmallLanguageModelCheckpointMetadata {
    if (!isRecord(value)) {
    ...
    const normalizedOptions: SmallLanguageModelOptions = {
    ...
    const variables = value.variables.map(validateCheckpointVariable)
...
function validateCheckpointVariable(value: unknown): SmallLanguageModelCheckpointVariable {
    if (!isRecord(value)) {
...
function validateContext(model: SmallLanguageModel, inputTokenIds: readonly number[]): void {
...
function validateModelOptions(options: SmallLanguageModelOptions): void {
...
function validateModelShape(model: SmallLanguageModel): void {
...
function validateTrainingOptions(options: SmallLanguageModelTrainingOptions): void {
